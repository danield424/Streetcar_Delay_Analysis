...
# Data {#sec-data}
  ...
  The data was thoroughly cleaned to ensure accuracy and prepare it for analysis; the process is detailed in @sec-appendix-a. In @sec-2-1 and @sec-2-2 we explore the dataset variables before conducting further analysis in @sec-results.
...
For further data exploration, we look at the hour, day of the week, month, and year. Details for acquiring this data are in @sec-appendix-c. Since the sample size for reduced and night service lines are much smaller than regular service lines, little information is gained from analyzing their specific time periods, so we will look at regular service data.
...
# Data Cleaning {#sec-appendix-a}
This included constructing a variable to determine the type of service - regular, reduced, night - of each specific streetcar line, and filtering observations to only include streetcar lines currently in service according to the Toronto Transit Commission [@ttc]. Additionally, delays under 2 minutes were removed and considered negligible, as were delays over 2 hours, as these were likely to be announced stoppages. Other information in the dataset such as station location, incident type, bound, and vehicle number were not discussed in this analysis. Data was cleaned using R [@citeR] and packages tidyverse [@tidyverse] and janitor [@janitor]. Data was analyzed with lubridate [@lubridate], tidyverse [@tidyverse], patchwork [@patchwork], and knitr [@knitr].

# Incomplete Data {#sec-appendix-b}
